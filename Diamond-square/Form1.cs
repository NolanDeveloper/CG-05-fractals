...
namespace Diamond_square
{
    public partial class Form1 : Form
    {
        ...
        private void midpoint(LinkedList<Point2D> vector, LinkedListNode<Point2D> hl, LinkedListNode<Point2D> hr, double r, int iter)
        {
            ...
            var len = Math.Sqrt((hr.Value.X - hl.Value.X) * (hr.Value.X - hl.Value.X) - (hr.Value.Y - hl.Value.Y) * (hr.Value.Y - hl.Value.Y));
            
            // var h = (hl.Value.Y + hr.Value.Y) / 2 + rand.Next((int)(-r * len), (int)(+r * len))
