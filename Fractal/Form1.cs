...
namespace affine_transformations
{
    public partial class Form1 : Form
    {
        ...
        //task 5
        private int size = 10;
        private Point2D this_p = new Point2D(0, 200);
        private List<char> fract_list = new List<char>();
        private List<string> turn_list = new List<string>();

        private int turn;
        private string dir;
        private List<char> rul = new List<char>();// = true; // true -- right, false -- left 

        public Form1()
        {
            InitializeComponent();
            // task5 ---
            FileStream file1 = new FileStream("L.txt", FileMode.Open);
            StreamReader reader = new StreamReader(file1);
            String text = reader.ReadToEnd();
            reader.Close();
            textBox1.Text = text;

            string[] s_list = text.Split('<');
            string c_char = s_list[1].Substring(0, s_list[1].Length - 1);
            fract_list.Add('F');
            turn = int.Parse(s_list[2].Substring(0, s_list[2].Length - 1));
            dir = s_list[3].Substring(0, s_list[3].Length - 1);

            for (int i = 2; i < s_list[4].Length - 1; i++)
                rul.Add(s_list[4][i]);
        ...
        {
            int size = fract_list.Count;
            for (int i = 0; i < fract_list.Count; i++)
            {
                if (fract_list[i] == 'F')
                {
                    fract_list.RemoveAt(i);
                    fract_list.InsertRange(i, rul);
                    i += rul.Count - 1;
                }
            }
            create_fract();
        ...
        private void create_fract()
        {
            graphics.Clear(Color.White);
            int left  = 0;// dir_list = new List<string>();
            int right = 0;
            string res = "";
            for (int i = 0; i < fract_list.Count; i++)
            {
                ...
                if (fract_list[i] == 'F')
                {
                    Edge edge = new Edge(this_p, new Point2D(this_p.X + size, this_p.Y));

                    var center = this_p;
                    var moveToCenter = Transformation.Translate(-center.X, -center.Y);

                   // MessageBox.Show("" + right);
                    for (int j = 0; j < left%6; j++)
                    {
                       // MessageBox.Show("" + j);
                        Transformation rotate = Transformation.Rotate((float)(Math.PI / 180 * -60));
                        var moveBack = Transformation.Translate(center.X, center.Y);
                        edge.Apply(moveToCenter * rotate * moveBack);
                    }

                    for (int j = 0; j < right%6; j++)
                    {
                      //  MessageBox.Show("" + j);
                        Transformation rotate = Transformation.Rotate((float)(Math.PI / 180 * 60));
                        var moveBack = Transformation.Translate(center.X, center.Y);
